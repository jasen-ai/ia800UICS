...
class ExcelDataReader:
    ...
    def batch_generate_images_from_prompts(
        ...
        Args:
            ...
            generator_type: 生成器类型（默认: "comfyui"）
        ...
        return batch_generate_images_from_excel_data(
            ...
            reference_image_dir=reference_image_dir or output_dir,
        )
    ...
    def batch_generate_videos_from_prompts(
        ...
        Args:
            ...
            reference_audio_dir: 参考音频目录（数字人 s2v）；指定时每分镜在该目录下找 {分镜号}.wav 等，调用 act_video_wan2_2_14B_s2v 工作流
...
def main():
    ...
    parser.add_argument('--scene-image-dir', type=str, default=None,
                       help='场景图目录，下存 {场景id}.png，用于参考图1（有参考图时用 Image Edit 工作流）')
    parser.add_argument('--character-image-dir', type=str, default=None,
                       help='角色图目录，下存 {角色id}.png，用于参考图2')
    parser.add_argument('--reference-image-dir', type=str, default=None,
                       help='图像汇总「参考图」列为相对路径时的基准目录（默认用 output-dir）')
    
    # 批量生成视频相关参数
    parser.add_argument('--generate-videos', action='store_true',
                       help='批量生成视频（使用图像汇总表中的视频提示词）')
    parser.add_argument('--generate-digital-human-videos', action='store_true',
                       help='批量生成数字人视频（图+音频→视频，使用 act_video_wan2_2_14B_s2v-aigc-api.json；需配合 --video-input-audio-dir）')
    ...
    parser.add_argument('--video-input-audio-dir', type=str, default=None,
                       help='数字人视频的输入音频目录，下存 {分镜号}.wav 或 {分镜号}.mp3 等；与 --generate-digital-human-videos 配合使用')
    parser.add_argument('--video-workflow', type=str, default=None,
                       help='视频生成工作流JSON文件路径（可选，默认使用 act_video_wan2_2_14B_i2v-aigc-api.json，仅用于comfyui类型；数字人模式固定使用 s2v 工作流）')
    ...
    try:
        ...
        if args.generate_videos:
            ...
            print("\n开始批量生成视频...")
            # 图生视频的输入图片目录：优先 --video-input-image-dir，否则 --reference-image-dir，否则视频输出目录
        ...
        # 如果需要生成数字人视频（图+音频→视频，s2v 工作流）
        if getattr(args, 'generate_digital_human_videos', False):
            if not VIDEO_GENERATOR_AVAILABLE:
                logger.error("视频生成器模块不可用，无法生成数字人视频。请确保video_generator.py存在")
                return reader, data
            video_audio_dir = getattr(args, 'video_input_audio_dir', None)
            if not video_audio_dir or not os.path.isdir(video_audio_dir):
                logger.error("数字人视频生成需要指定已存在的音频目录: --video-input-audio-dir（目录下存放 {分镜号}.wav 或 .mp3 等）")
                return reader, data
            
            print("\n开始批量生成数字人视频（图+音频 → 视频，act_video_wan2_2_14B_s2v-aigc-api.json）...")
            video_ref_dir = getattr(args, 'video_input_image_dir', None) or args.reference_image_dir or args.video_output_dir
            digital_results = reader.batch_generate_videos_from_prompts(
                output_dir=args.video_output_dir,
                comfyui_server=args.comfyui_server,
                workflow_path=None,
                episode_filter=args.episode,
                ...
                fps=args.fps,
                reference_image_dir=video_ref_dir,
                reference_audio_dir=video_audio_dir,
                generator_type=args.video_generator_type,
                enable_prompt_expansion=args.enable_video_prompt_expansion,
                sora_api_key=args.sora_api_key,
                sora_host=args.sora_host,
            ...
            print(f"\n✓ 数字人视频生成完成! 共处理 {len(digital_results)} 个分镜")
